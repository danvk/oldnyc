...
class FileStatus(TypedDict):
    ...
    batch: dict  # Optional[openai.types.Batch]
...
def is_failed_batch(status: BatchStatus):
    return status == 'cancelled' or status == 'cancelling' or status == 'expired' or status == 'failed'
    ...
    for batch_file in batch_files:
        ...
        def check_fail_status():
            if is_failed_batch(batch_status):
                sys.stderr.write(f'{batch_file}: {batch_id} batch failed: {batch_status}\n')
                sys.stderr.write(f'Check https://platform.openai.com/batches/{batch_id}\n')
                sys.exit(1)
